...
#perl looping.pl freq_pwdset_ovr.txt $m_tot $numloops $B_fraction $num_of_dictionaries $dicts1 $dict2 .. $q_actual1 $q_actual2
#perl looping.pl freq_pwdset_ovr.txt 10 3 0.1 2 freq_d0_ovr.txt freq_d1_ovr.txt 0.3 0.7
...
for(my $i=1; $i<=$num_of_dictionaries; $i++){
	$actual_q[$i-1] = $ARGV[$i+4+$num_of_dictionaries];
}
...
foreach $reset_q (@reset_q){
	...
	foreach $guess_choice (@guess_choice){
		...
		$loop=0;
		while($loop <$numloops){
			system("perl gradient_descents.pl $reset_q $guess_choice $m_tot $loop $B_fraction $pwdset $num_of_dictionaries @dicts @actual_q");
...
while($loop <$numloops){
	...
	plot_Q($m_tot, $B_fraction, $pwdset_name, $loop, $num_of_dictionaries, \@actual_q, \@dicts); # plots the Q values. success
	system("gnuplot -c plot_QValues_mult_m_for_known_qs.gp $m_tot $B_fraction $pwdset_name $loop $num_of_dictionaries @actual_q");
	system("gnuplot -c plot_QValues_mult_m_plot_diff_for_known_qs.gp $m_tot $B_fraction $pwdset_name $loop $num_of_dictionaries @actual_q");
...
do for [k=1:$num_dicts]{
	if (k == 1){
    	set arrow 1 from graph 0, first $q_act[0] to graph 1,first $q_act[0] nohead lc rgb "#000000" front
	}
	if (k == 2){
    	set arrow 2 from graph 0, first $q_act[1] to graph 1,first $q_act[1] nohead lc rgb "#000000" front
	}
	if (k == 3){
    	set arrow 3 from graph 0, first $q_act[2] to graph 1,first $q_act[2] nohead lc rgb "#000000" front
	}
	if (k == 4){
    	set arrow 4 from graph 0, first $q_act[3] to graph 1,first $q_act[3] nohead lc rgb "#000000" front
	}
}
