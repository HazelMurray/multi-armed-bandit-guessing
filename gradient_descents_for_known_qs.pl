...
#perl gradient_descents.pl bestqs guess_best_byQ 10 0 0.1 freq_pwdset_ovr.txt 2 freq_d0_ovr.txt freq_d1_ovr.txt 0.3 0.7
...
sub Likelihood{
	...
	if (1-$sum_Q<$stopping_sum_Q){
		print "We have learnt all we can about the distribution of the pwdset because we have sampled everything. sum Q == $sum_Q.\n";	#note this ends before the last guess. But it isn't really a gradient descent problem if we have tested every sample. 
		exit();
	}
...
sub guess_best_byQ{
	...
	my %Q_all;
	foreach $dict (@dicts){#would need to sort and take top one. 
